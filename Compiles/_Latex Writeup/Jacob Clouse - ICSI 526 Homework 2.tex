...
\newline
 ...
 \begin{enumerate}
 	\item \textbf{MSB of S in LSB of H (ORIGINAL)}: The host degrades slowly as it is only affecting lsb. Host image become apparent around 3 to 4 bits (more rapid as it is MSB).
 	\item \textbf{MSB of S in MSB of H}: We see that the changes to the host file are much more rapid than in the original MSB S to LSB H (this is because we are overwriting the MSB of the host), secret image appears just as quickly (again, because we are using MSB).
 	\item \textbf{LSB of S in LSB of H}: Host file again degrades slowely (as it is using LSB of H), and the secret image appears much slower than in the original (because it is using LSB).
