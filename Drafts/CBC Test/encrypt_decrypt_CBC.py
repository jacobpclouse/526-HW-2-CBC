...
for i in range(block_size, len(ciphertext), block_size):
    ...
    decrypted_block = bytes([a^b for a,b in zip(decryptor.update(block), ciphertext[i-block_size:i])])
    decrypted_blocks.append(decrypted_block)
decrypted_blocks.append(decryptor.finalize())

# Remove PKCS#7 padding
unpadder = padding.PKCS7(128).unpadder()
plaintext = unpadder.update(b"".join(decrypted_blocks)) + unpadder.finalize()
