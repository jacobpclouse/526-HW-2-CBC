...
def ImageHiding(H_path, S_path, output_path):
    ...
    # Iterate over each pixel of the images and hide the LSB of S in the MSB of H
    for i in range(H.size[0]):
        for j in range(H.size[1]):
            H_r, H_g, H_b = H_pixels[i, j]
            S_r, S_g, S_b = S_pixels[i, j]

            # Hide the LSB of S in the MSB of H
            H_r = (H_r & 254) | ((S_r & 128) >> 7)
            H_g = (H_g & 254) | ((S_g & 128) >> 7)
            H_b = (H_b & 254) | ((S_b & 128) >> 7)
